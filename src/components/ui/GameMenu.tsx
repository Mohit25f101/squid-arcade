  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const s of streaks) {
        ...
        s.x += s.speed; if (s.x > canvas!.width + s.length) { s.x = -s.length; s.y = Math.random() * canvas!.height; }
      }
...
function GameIconCard({ mode, onHover, onClick, delay }: { mode: GameMode; onHover: () => void; onClick: (id: GameId) => void; delay: number }) {
  const lines = mode.label.split("\n");
  const colorKey = mode.color === "#FF0066" ? "pink" : mode.color === "#00FFB2" ? "teal" : "gold";
  return (
    <button className="sq-icon-card" data-color={colorKey} style={{ animationDelay: `${delay}s` }} onMouseEnter={onHover} onClick={() => onClick(mode.id)} aria-label={`Select ${mode.sub}`}>
      <span className="sq-icon-badge" style={{ color: mode.color, background: `${mode.color}12`, border: `1px solid ${mode.color}30` }}>{mode.badge}</span>
      <img src={mode.icon} alt={mode.sub} className="sq-icon-img" style={{ color: mode.color }} />
      <span className="sq-icon-label" style={{ color: mode.color }}>{lines.map((l, i) => <React.Fragment key={i}>{l}{i < lines.length - 1 && <br />}</React.Fragment>)}</span>
    </button>
  );
}

function SelectCard({ mode, onHover, onClick }: { mode: GameMode; onHover: () => void; onClick: (id: GameId) => void }) {
  const colorKey = mode.color === "#FF0066" ? "pink" : mode.color === "#00FFB2" ? "teal" : "gold";
  return (
    <button className="sq-select-card" data-color={colorKey} onMouseEnter={onHover} onClick={() => onClick(mode.id)} aria-label={`Play ${mode.sub}`}>
      <img src={mode.icon} alt={mode.sub} className="sq-select-icon" style={{ color: mode.color }} />
      <div className="sq-select-content"><span className="sq-select-title" style={{ color: mode.color }}>{mode.sub}</span><span className="sq-select-desc">{mode.desc}</span></div>
      <div className="sq-select-meta"><span className="sq-select-badge" style={{ color: mode.color, background: `${mode.color}15`, border: `1px solid ${mode.color}40` }}>EP. {String(mode.episode).padStart(2, "0")}</span><span className="sq-select-cta" style={{ color: mode.color }}>ENTER →</span></div>
    </button>
  );
}
...
function ExitConfirm({ onCancel, onConfirm }: { onCancel: () => void; onConfirm: () => void }) {
  return (
    <div className="sq-overlay-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      <div className="sq-overlay-panel" style={{ textAlign: "center", maxWidth: 380 }}>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "1rem" }}><SymbolTrio size={18} gap={10} /></div>
        <h2 className="sq-overlay-title" style={{ fontSize: "clamp(1.8rem,4vw,2.6rem)", marginBottom: "0.5rem" }}>EXIT GAME?</h2>
        <p style={{ fontFamily: "var(--font-mono-sq)", fontSize: "0.72rem", letterSpacing: "0.15em", color: "rgba(255,255,255,0.4)", marginBottom: "1.5rem", lineHeight: 1.6 }}>Your progress will be lost.<br />Are you sure you want to exit?</p>
        <div className="sq-confirm-row"><button className="sq-btn sq-btn-cancel" onClick={onCancel}>CANCEL</button><button className="sq-btn sq-btn-confirm" onClick={onConfirm}>EXIT</button></div>
      </div>
    </div>
  );
}
...
export default function GameMenu({ onLaunch }: GameMenuProps = {}) {
  ...
  useEffect(() => {
    const id = setInterval(() => { setPlayerCount((p) => { const n = p - Math.floor(Math.random() * 3); return n < 1 ? 456 : n; }); setCountTick((t) => !t); }, 2200);
  ...
  const handleLaunch = useCallback((id: GameId) => {
    audio.play("transition"); audio.stopBg();
  ...
  return (
    <div className="sq-menu-root sq-scanlines">
      <BackgroundCanvas />
      <div className="sq-grid-bg" aria-hidden /><div className="sq-scanline-sweep" aria-hidden />
      ...
      <div className="sq-menu-content" style={{ opacity: menuState === "intro" ? 0 : 1, transition: "opacity 0.5s ease" }}>
        ...
        {menuState !== "select" && (
          <div className="sq-icons-row">
            {GAME_MODES.map((mode, i) => <GameIconCard key={mode.id} mode={mode} onHover={handleHover} onClick={menuState === "main" ? handleShowSelect : handleLaunch} delay={0.65 + i * 0.12} />)}
          </div>
        )}
        {menuState === "select" && (
          <div style={{ width: "100%", animation: "sq-slide-up 0.6s cubic-bezier(0.16,1,0.3,1) both" }}>
            <button onClick={handleBackToMain} onMouseEnter={handleHover} style={{ all: "unset", cursor: "pointer", fontFamily: "var(--font-mono-sq)", fontSize: "0.68rem", letterSpacing: "0.25em", color: "rgba(255,0,102,0.5)", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "0.4rem", transition: "color 200ms" }}>← BACK</button>
            <div className="sq-select-grid">
              {GAME_MODES.map((mode) => <SelectCard key={mode.id} mode={mode} onHover={handleHover} onClick={handleLaunch} />)}
            </div>
          </div>
        )}
        {menuState === "main" && (
          <nav className="sq-nav-list" aria-label="Main menu navigation">
            <button className="sq-nav-item" onMouseEnter={handleHover} onClick={() => { handleClick(); handleShowSelect(); }}><span className="sq-nav-item-label sq-font-bebas">PLAY GAME</span><span className="sq-nav-item-arrow">→</span></button>
            <button className="sq-nav-item" onMouseEnter={handleHover} onClick={() => { handleClick(); handleOpenSettings(); }}><span className="sq-nav-item-label sq-font-bebas">SETTINGS</span><span className="sq-nav-item-arrow">→</span></button>
            <button className="sq-nav-item danger" onMouseEnter={handleHover} onClick={() => { handleClick(); handleExitRequest(); }}><span className="sq-nav-item-label sq-font-bebas">EXIT</span><span className="sq-nav-item-arrow" style={{ color: "rgba(255,80,80,0.4)" }}>→</span></button>
          </nav>
        )}
        {menuState === "main" && (
          <div className="sq-status-pill">
            <div className="sq-status-item"><span className="sq-status-label sq-font-mono">PLAYERS</span><span className="sq-status-val sq-font-bebas sq-neon-teal" style={{ color: "#00FFB2", transform: countTick ? "scale(1.08)" : "scale(1)", transition: "transform 0.4s" }}>{playerCount}</span></div>
            <div className="sq-status-divider" aria-hidden />
            <div className="sq-status-item"><span className="sq-status-label sq-font-mono">PRIZE</span><span className="sq-status-val sq-font-bebas sq-neon-gold" style={{ color: "#FFD700" }}>₩45.6B</span></div>
            <div className="sq-status-divider" aria-hidden />
            <div className="sq-status-item"><span className="sq-status-label sq-font-mono">TIMER</span><span className="sq-status-val sq-font-mono sq-neon-pink" style={{ color: "#FF0066", fontSize: "1.2rem" }}>00:{String(countdownSec).padStart(2, "0")}</span></div>
          </div>
        )}
        <div style={{ opacity: 0.22, animation: "sq-fade-in 0.8s ease 1.6s both" }}><SymbolTrio size={10} gap={8} glow={false} /></div>
      </div>
      {menuState === "settings" && <Suspense fallback={null}><SettingsOverlay onClose={handleCloseSettings} onHover={handleHover} onClick={handleClick} /></Suspense>}
      {menuState === "exit-confirm" && <ExitConfirm onCancel={() => { handleClick(); setMenuState("main"); }} onConfirm={handleExitConfirm} />}
      <div className="portrait-warning" role="alert"><div className="portrait-warning-icon">📱</div><p style={{ fontFamily: "var(--font-mono-sq)", letterSpacing: "0.1em" }}>ROTATE YOUR DEVICE</p></div>
    </div>
  );
}